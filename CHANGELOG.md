...
#### chat_messages 테이블 신규 생성
- id / channel_key(예: 'world', 'building:bar') / user_id / content / created_at
- 월드 채팅과 건물별 채팅을 하나의 테이블에서 channel_key로 구분
- Realtime 활성화 필요: `alter publication supabase_realtime add table chat_messages;`

### 신규 파일

#### js/ui/chat.js
- 재사용 가능한 공용 채팅 위젯 (우측 하단 💬 플로팅 버튼)
- Supabase Realtime으로 새로고침 없이 실시간 메시지 수신
- 건물 11개 페이지(술바·양조장·커뮤니티·도서관·영상관·안내소·팝업광장·내공간·상점·마리아주·소버카페)에 각 건물 전용 채팅으로 적용, 건물 테마 색상에 맞춰 버튼 색 다르게 적용

### 변경 파일

#### index.html (월드 채팅)
- 기존에 있던 더미(가짜) 하단 채팅 UI의 "월드" 탭을 chat_messages 실데이터로 교체
- 건물/커뮤니티/DM 탭은 계획대로 더미 유지 (각각 팔로우 시스템, 커뮤니티 시스템 완성 후 진행 예정)
- 채팅 입력창에서 키 입력(q/e/r 등)이 3D 마을 조작으로 새어나가던 버그 수정 (`stopPropagation` 추가)
- 채팅 탭을 이동했다가 "월드"로 복귀 시 이전 탭 내용이 남아있던 버그 수정 (메시지 캐시 방식으로 재구성)
- 처음에 별도로 추가했던 중복 플로팅 채팅 버튼 제거 (기존 하단 채팅 UI와 겹쳐서 정리)

### 배포 이슈 해결 (Cloudflare)
- `wrangler.jsonc` 파일을 저장소 루트에 추가 — Cloudflare가 Pages에서 Workers(정적 자산 서빙) 방식으로 플랫폼 차원에서 전환하는 과정에서 설정 파일이 누락되어 배포가 계속 실패하던 문제 해결
- 배포 주소가 `meokgallie.pages.dev`에서 `meokgallie.ktyear.workers.dev`로 변경되어 있음을 확인

### 기술 결정
- DM(개인 메시지), 커뮤니티 채팅은 신고/차단 등 안전장치와 커뮤니티 시스템 완성이 선행되어야 해서 이번 범위에서 제외
- "창문 밝기가 접속자 수에 따라 변하는" 연출은 3D 마을(index.html) 렌더링 쪽 작업이라 별도 세션으로 분리
